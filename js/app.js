...
const App = (() => {
  ...
  function _initBookmarklet() {
    ...
    // Inline bookmarklet — finds processing ID from page's already-loaded network requests
    const bookmarklet = `javascript:(function(){'use strict';if(!window.location.hostname.includes('cropster.com')){alert("Socky1: Ouvrez cette page sur Cropster d'abord.");return}const o=document.createElement('div');o.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:99999;display:flex;align-items:center;justify-content:center;font-family:system-ui';o.innerHTML='<div style="background:%23231e19;color:%23f0e6dc;padding:2rem 3rem;border-radius:12px;text-align:center;border:2px solid %23d4915e"><h2 style="color:%23d4915e;margin:0 0 .5rem">Socky1</h2><p id=socky1-s>Recherche du roast...</p></div>';document.body.appendChild(o);const s=document.getElementById('socky1-s');let p=null;const entries=performance.getEntriesByType('resource');for(const e of entries){const url=decodeURIComponent(e.name);const m=url.match(/processingCurves.*\\[processing\\]=([a-zA-Z0-9]+)/);if(m){p=m[1];break}};if(!p){for(const e of entries){const url=decodeURIComponent(e.name);const m=url.match(/\\/api\\/v2\\/processings\\/([a-zA-Z0-9]+)/);if(m){p=m[1];break}}};if(!p){o.remove();p=prompt("Socky1: ID introuvable. Ouvrez DevTools (F12) > Network > cliquez sur processingCurves > regardez l'URL pour trouver l'ID après 'processing='\\nEntrez-le ici:");if(!p)return}s.textContent='Chargement ('+p+')...';const b='https://c-sar.cropster.com/api/v2',h={Accept:'application/vnd.api+json;charset=UTF-8'};Promise.all([fetch(b+'/processingCurves?filter[processingCurves][processing]='+p,{headers:h,credentials:'include'}).then(r=>r.json()).catch(()=>null),fetch(b+'/processingMeasures?filter[processingMeasures][processing]='+p,{headers:h,credentials:'include'}).then(r=>r.json()).catch(()=>null),fetch(b+'/processings/'+p,{headers:h,credentials:'include'}).then(r=>r.json()).catch(()=>null)]).then(([c,me,pr])=>{if(!c||!c.data||!c.data.length){alert('Socky1: Aucune courbe pour ID '+p+'.');o.remove();return}const e={socky1Export:true,version:1,exportedAt:new Date().toISOString(),processingId:p,curves:c,measures:me,processing:pr};let f='socky1_'+p;if(pr&&pr.data&&pr.data.attributes&&pr.data.attributes.startDate)f='socky1_'+pr.data.attributes.startDate.split('T')[0]+'_'+p;const bl=new Blob([JSON.stringify(e)],{type:'application/json'}),ul=URL.createObjectURL(bl),a=document.createElement('a');a.href=ul;a.download=f+'.json';document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(ul);const n=c.data.find(x=>x.attributes&&x.attributes.name==='beanTemperature');s.textContent='Téléchargé! '+(n?n.attributes.values.length+' points BT':'');setTimeout(()=>o.remove(),2500)}).catch(x=>{alert('Socky1: Erreur - '+x.message);o.remove()})})()`;
